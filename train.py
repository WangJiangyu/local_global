...
from models.args import ArgumentParserRGBTSegmentation
from models.ESANet import ESANet
...
def train(args, epo, model, train_loader, optimizer):
    model.train()
    for it, (images, labels, names) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        logits = model(images)
        loss = F.cross_entropy(logits[0][0], labels)  # Note that the cross_entropy function has already include the softmax function
        loss_8 = F.cross_entropy(logits[0][1], labels) + F.cross_entropy(logits[1][1], labels)
        loss_16 = F.cross_entropy(logits[0][2], labels) + F.cross_entropy(logits[1][2], labels)
        loss_32 = F.cross_entropy(logits[0][3], labels) + F.cross_entropy(logits[1][3], labels)
        loss_thermal = F.cross_entropy(logits[1][0], labels)
        total_loss = 2*loss+0.8*loss_8+0.6*loss_16+0.4*loss_32+2*loss_thermal
        # total_loss = loss + loss_thermal
        total_loss.backward()
        optimizer.step()
        lr_this_epo = 0
        ...
        print('Train: %s, epo %s/%s, iter %s/%s, lr %.8f, %.2f img/sec, loss %.4f, time %s' \
            % (args.model_name, epo, args.epochs, it+1, len(train_loader), lr_this_epo, len(names)/(time.time()-start_t), float(total_loss),
        ...
        if accIter['train'] % 500 == 0:
            if view_figure:
                ...
                # prediction = logits[0].argmax(1).cpu().numpy().squeeze().flatten()
                ...
                predicted_tensor = logits[0][0].argmax(1).unsqueeze(1) * scale # mini_batch*args.n_class*480*640 -> mini_batch*480*640 -> mini_batch*1*480*640
...
def validation(args, epo, model, val_loader):
    ...
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(val_loader):
            ...
            start_t = time.time() # time.time() returns the current time
            logits = model(images)
            loss = F.cross_entropy(logits[0], labels) + F.cross_entropy(logits[1], labels)  # Note that the cross_entropy function has already include the softmax function
...
def testing(args, epo, model, test_loader):
    ...
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(test_loader):
            ...
            logits = model(images)
            label = labels.cpu().numpy().squeeze().flatten()
            prediction = logits[0].argmax(1).cpu().numpy().squeeze().flatten() # prediction and label are both 1-d array, size: minibatch*640*480
    ...
    if epo == 0:
        with open(testing_results_file, 'w') as f:
            f.write("# %s, backbone: %s, initial lr: %s, batch size: %s, date: %s \n" %(args.model_name, args.backbone, args.lr, args.batch_size, datetime.date.today()))
...
if __name__ == '__main__':
    ...
    model = eval(args.model_name)(n_class=args.n_class, encoder_rgb='mit_b1', encoder_thermal=args.backbone)
    if args.gpu >= 0:
        model.cuda(args.gpu)
    ...
    for epo in range(args.epochs_from, args.epochs):
        ...
        testing(args, epo, model, test_loader) # testing is just for your reference, you can comment this line during training
