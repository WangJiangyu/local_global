...
def build_model(args, n_classes):
    ...
    if args.modality == 'rgbd':
        # use the same encoder for thermal encoder and rgb encoder if no
        # specific thermal encoder is provided
        if args.encoder_thermal in [None, 'None']:
            args.encoder_thermal = args.encoder

        model = ESANet(height=args.height, width=args.width, n_class=n_classes, encoder_rgb=args.encoder,
                       encoder_thermal=args.encoder_thermal, encoder_block=args.encoder_block,
                       channels_decoder=channels_decoder, pretrained_on_imagenet=pretrained_on_imagenet,
                       pretrained_dir=args.pretrained_dir, activation=args.activation,
                       encoder_decoder_fusion=args.encoder_decoder_fusion, context_module=args.context_module,
                       nr_decoder_blocks=nr_decoder_blocks,
                       fuse_thermal_in_rgb_encoder=args.fuse_thermal_in_rgb_encoder, upsampling=args.upsampling)
        ...
        model = ESANetOneModality(
            ...
            weighting_in_encoder=args.fuse_thermal_in_rgb_encoder,
    ...
    if args.he_init:
        ...
        # thermalwise convolutions (currently only used in learned upsampling)
    ...
    if args.finetune is not None:
        ...
        for name, param in model.named_parameters():
            if 'encoder_rgb' in name or 'encoder_thermal' in name or 'se_layer' in name:
