...
class NonLocalAttention(nn.Module):
    def __init__(self, channel, reduction=8, ksize=1, scale=3, stride=1, softmax_scale=40, average=True):
        ...
        self.conv_match1 = BasicBlock(channel, channel // reduction, kernel_size=ksize, bn=False, act=nn.PReLU())
        self.conv_match2 = BasicBlock(channel, channel // reduction, kernel_size=ksize, bn=False, act=nn.PReLU())
        self.conv_assembly = BasicBlock(channel, channel, kernel_size=ksize, bn=False, act=nn.PReLU())

    def forward(self, rgb, thermal):
        x_embed_1 = self.conv_match1(rgb)
        x_embed_2 = self.conv_match2(thermal)
        x_assembly = self.conv_assembly(thermal)
        ...
        x_final = torch.matmul(soft_score, x_assembly)
        return x_final.permute(0, 2, 1).view(N, -1, H, W) + rgb
